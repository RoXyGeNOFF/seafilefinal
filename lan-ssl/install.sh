...
mkdir -p "${DATA_DIR}/seafile" "${DATA_DIR}/mysql" "./nginx/certs"

# self-signed SSL
if [ ! -f "./nginx/certs/seafile.key" ]; then
  openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
    -keyout ./nginx/certs/seafile.key -out ./nginx/certs/seafile.crt \
    -subj "/C=RU/ST=Local/L=LAN/O=Seafile/OU=Dev/CN=${LOCAL_IP}"
fi
...
echo -e "\n${GREEN}${BOLD}Seafile (LAN SSL) поднят.${RESET} Откройте: https://${LOCAL_IP}/"
